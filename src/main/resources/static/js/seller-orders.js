			...
			$.get("fetchPendingOrders",function(data,status){
				function formatItem(item) {
					...
					if(item.status=="pending"){
						...
						return '<td><input class="chkRow" type="checkbox"/></td><td>'+item.orderno + '</td> <td> ' + item.date + ' </td><td>' + item.customer_name+'</td><td>'+item.price+'</td><td>'+item.quantity+"</td><td>"+item.pincode+"</td><td>"+item.paymentmode+"</td><td>Reason to be done</td>";
					}
					}
...
$(function() {
	...
	$("#bulk-confirm-btn").on('click',function(){
		...
		bulkOrderDispatch.dialog("open");

	})
	bulkOrderDispatch = $("#bulkOrderDispatchAction").dialog({
			autoOpen: false,
	    width: 350,
	    modal: true,
	    buttons: {
	    	Yes: function(){
	    		console.log("inside" + ordernos)
	    		$.ajax({
	    			url:"bulkorderaction",
	    			type:"POST",
	    			datatype:'json',
	    			data:{ordernos:ordernos,status:"completed"},
	    			success:function(){
	    			
	    				$("#bulkOrderDispatchAction").dialog("close");
	...
	bulkOrderConfirm = $("#bulkOrderAction").dialog({
		autoOpen: false,
	    width: 350,
	    modal: true,
	    buttons: {
	    	Yes: function(){
	    		...
	    		$.ajax({
	    			...
	    			type:"POST",
	    			datatype:'json',
	    			data:{ordernos:ordernos,status:"active"},
	    			success:function(){
